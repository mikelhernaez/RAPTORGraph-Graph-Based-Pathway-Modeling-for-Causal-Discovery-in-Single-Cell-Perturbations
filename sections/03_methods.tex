...
In models with dense encoders, one can enforce acyclicity in the causal graph with simple structural constraints, such as an upper-triangular adjacency matrix, and thus assume an implicit topological ordering of the latent factors (at the cost of the spillover problem). However, due to the fixed mapping of the preconditioned GraphPathway encoder, we no longer can assume such topological ordering as it would not match the biology. We therefore adapt the DAGMA framework~\citep{bello2022dagma} to learn the latent causal structure in our deep learning setting.
...
an interventional prediction loss ($\mmdloss$) to align distributions of predicted perturbed gene expressions with  experimental ground truth; 
and a causal graph loss ($\dagmaloss$) to enforce structural acyclicity:
\begin{equation}
