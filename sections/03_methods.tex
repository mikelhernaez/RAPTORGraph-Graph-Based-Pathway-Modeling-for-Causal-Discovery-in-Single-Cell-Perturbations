...
The other submatrices are learnable and dense, allowing the model to discover complex gene-gene relationships and associate them to the learned \cmps{}. %Crucially, this preconditioned structure  specified which \cmp{} to shift for each intervention, to be predefined and fixed rather than learned, .
...
\textbf{Decoder.} As last step of the model, a non-linear dense decoder maps these causal states back to the high-dimensional gene expression space to predict the post-perturbation (or unperturbed/control) gene expression. 
...
In models with dense encoders, one can enforce acyclicity in the causal graph with simple structural constraints, such as an upper-triangular adjacency matrix, by assuming an implicit topological ordering of the latent factors (at the cost of the spillover problem). However, as the preconditioned GraphPathway encoder fixes the mapping from perturbed genes to specific causal latent factors, we can no longer assume a topological ordering of the latent factors. Note that such imposed arbitrary hierarchy would not match the biology. We therefore adapt the DAGMA framework~\citep{bello2022dagma} to learn the latent causal structure in our deep learning setting.
...
However, this random pairing may match cells with poor counterfactuals, providing inconsistent gradient signals that force the model to minimize loss by predicting the average of the target distribution---a phenomenon termed \textit{mean collapse} (see \cref{fig:app:ot_comparison}).
...
an interventional prediction loss ($\mmdloss$) to align distributions of predicted perturbed gene expressions the experimental ground truth; 
