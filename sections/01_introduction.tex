...
Our first key innovation is the GraphPathway layer, a novel architecture that uses preconditioning to enforce sparse mappings from genes to learned interpretable causal factors, which we term \cmps{}.
This design enables the model to perform the clean, single-node latent associations required for causal inference, while  maintaining interpretability.
This innovation is complemented by an \gls{ot} preprocessing step, which mitigates the distinct problem of mean collapse.

\revised{We benchmark the generative quality of \glsentryshort{raptorgraph} on the \citet{norman2019exploring} Perturb-seq dataset and the \citet{replogle_combinatorial_2020} CRISPRi dataset, assess the accuracy of inferred genetic interactions under combinations of perturbations, and evaluate the biological interpretability of the inferred \cmps{}. We uncover biologically accurate programs in the K562 cell line, including the differentiation-induced cell cycle arrest driven by EGR1 and the stress-induced growth arrest mediated by JUN.}
