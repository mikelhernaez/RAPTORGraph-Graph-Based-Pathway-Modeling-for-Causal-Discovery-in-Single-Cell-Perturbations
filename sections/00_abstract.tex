...
% Causal representation learning has shown great promise for predicting unseen combinations of perturbations, but existing methods often violate the theoretical requirements for identifiability guarantees.  
...
Experiments involving perturbation of single cells are central to understanding cellular mechanisms and accelerating therapeutic discovery.
However, the space of combinatorial perturbations is intractably large.
While Causal Representation Learning (CRL) holds promise for predicting unseen perturbations, existing methods struggle to bridge theory and practice.
Crucially, they often fail to mathematically capture the complex nature of biological responses, thereby violating the theoretical requirements for identifiability.
We introduce \glsentryshort{raptorgraph}, an end-to-end CRL framework that tackles the critical gap in many existing methods: soft interventions.
\glsentryshort{raptorgraph} addresses these issues through: i) a preconditioned GraphPathway encoder that enforces soft intervention-guided mappings to \cmps{}, enabling clean single node interventions for causal identifiability, and ii) optimal transport that aligns control and perturbed populations to stabilize conditional generation.
Empirical results show that \glsentryshort{raptorgraph} improves both reconstruction fidelity and distributional matching across datasets, yields superior performance in predicting non-additive combinatorial effects, and proves highly effective in uncovering key genetic drivers from observed phenotypes.
Finally, we show that \glsentryshort{raptorgraph} recovers biologically meaningful latent programs, providing an bridge between generative quality and mechanistic insights.
\end{abstract}
% \begin{abstract}
%
